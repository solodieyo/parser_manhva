...
async def user_subscribe(callback: CallbackQuery, session: AsyncSession, callback_data: ManhvaSubscribeCallback):
    manhva_id = callback_data.manhva_id
    try:
        await add_manhva(
            _id=manhva_id, session=session, user_id=callback.from_user.id
        )
